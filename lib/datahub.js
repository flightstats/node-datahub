  ...
  _createClass(Datahub, [{
    ...
    value: function sendQueue() {
      if (this.config.queueEnabled && this.queue.curCount > 0) {
        ...
        var queueItems = [];
        _lodash2.default.forIn(queueItemsToParse, function (value, key) {
        ...
        _bluebird2.default.map(queueItems, function (queueItem) {
          return that.addContent(queueItem.channelName, queueItem.content).then(function (resp) {
            that.config.logger.log('Successfully added ' + queueItem.content.length + ' queued items to \'' + queueItem.channelName + '\'');
            return _bluebird2.default.resolve();
          }, function (err) {
            that.config.logger.error('Error adding queued items to \'' + queueItem.channelName + '\'', err.message ? err.message : err);
            return _bluebird2.default.resolve(); // don't reject whole queue if one queue item fails...
          });
        }, { concurrency: 10 }).then(function (resp) {
    ...
    key: 'addContentToQueue',
    value: function addContentToQueue(name, content) {
      if (name && content) {
    ...
    value: function getLatest(name, N) {
      ...
      return this._crud(this.config.url + '/channel/' + name + '/latest' + (N ? '/' + N : ''), 'GET');
    }
    ...
    value: function getEarliest(name, N) {
      ...
      return this._crud(this.config.url + '/channel/' + name + '/earliest' + (N ? '/' + N : ''), 'GET');
    }
    ...
    value: function createGroupCallback(config) {
      ...
      if (_lodash2.default.isBoolean(config.paused) && config.paused) {
        data.paused = true;
      }
      if (config.batch && (config.batch.toUpperCase() === 'SINGLE' || config.batch.toUpperCase() === 'MINUTE')) {
        data.batch = config.batch.toUpperCase();
      }
      if (_lodash2.default.isBoolean(config.heartbeat) && config.heartbeat) {
        data.heartbeat = true;
      }
    ...
    value: function updateGroupCallback(name, config) {
      ...
      if (_lodash2.default.isBoolean(config.paused)) {
        data.paused = config.paused;
      }
      if (_lodash2.default.isBoolean(config.heartbeat)) {
    ...
    value: function channelAlert(name, config) {
      ...
      if (isString(config.operator) && ['>=', '>', '==', '<', '<='].indexOf(config.operator) !== -1) {
